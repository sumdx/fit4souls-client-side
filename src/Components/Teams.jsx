...
const Teams = () => {
    ...
    return (
        <div className='container mx-auto my-32'>
            ...
            <div className='grid mt-10 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4'>
                {allTrainersData.map((data, index )=>{
                    if(index<4){
                        return(
                            <div className='p-4 flex flex-col border shadow rounded-xl justify-center items-center text-center gap-2'>
                                <img className="rounded-full mx-auto p-1 border-black border-2 w-36 h-36" src={data.photoUrl} ></img>
                                <h1 className='text-center mt-4 dark:text-white'>{data.name}</h1>
                                <span className=" mx-auto bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">Trainer</span>
                            </div>
                        )
                    }
